...
def _malloc_trim() -> None:
    """Принудительно возвращает освобождённые страницы glibc ядру (только Linux).

    После каждого прогона тестов glibc удерживает свободную память в своём
    пуле, не отдавая её ОС. malloc_trim(0) сигнализирует аллокатору вернуть
    все страницы выше brk-указателя. Позволяет снизить RES с ~120 МБ до
    ~40–50 МБ в фазе сна между прогонами.
    """
